...
namespace Avogadro {
                                         ...
                                         QString *error)
  {
    ...
    // Close the stream before renaming the temporary file. This is required
    // on Windows, where renaming an open file can fail.
    ofs.close();
    ...
    if (success) {
      ...
      if (error) {
        error->append(QObject::tr("Writing conformers to file '%1' failed - could not rename temporary file.")
                      .arg(fileName));
      }
    } else if (error) {
