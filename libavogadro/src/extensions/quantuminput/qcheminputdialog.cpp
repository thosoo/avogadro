    ...
    else if (m_molecule && m_coordType == ZMATRIX)
    {
      ...
      FOR_ATOMS_OF_MOL(atom, &obmol)
      {
        ...
        mol << qSetFieldWidth(4) << right
            ...
            << qSetFieldWidth(0);
        if (atom->GetIdx() > 1)
          mol << ' ' << QString(OpenBabel::OBElements::GetSymbol(a->GetAtomicNum())
                                + QString::number(a->GetIdx()))
              << " r" << atom->GetIdx();
        if (atom->GetIdx() > 2)
          mol << ' ' << QString(OpenBabel::OBElements::GetSymbol(b->GetAtomicNum())
                                + QString::number(b->GetIdx()))
              << " a" << atom->GetIdx();
        if (atom->GetIdx() > 3)
          mol << ' ' << QString(OpenBabel::OBElements::GetSymbol(c->GetAtomicNum())
