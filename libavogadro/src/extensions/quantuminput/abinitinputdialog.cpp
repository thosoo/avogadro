...
namespace Avogadro
{
  ...
  void AbinitInputDialog::writeSettings(QSettings &settings) const
  {
    ...
    settings.setValue("abinit/ToleranceType",
                      ui.toleranceTypeCombo->currentIndex());
  ...
  void AbinitInputDialog::readSettings(QSettings &settings)
  {
    ...
    ui.tsmearLineEdit->setText(settings.value("abinit/Tsmear",
                                              "0.01").toString());
    setTsmear();
    ...
    // Advanced Tab
    ui.tolmxfLineEdit->setText(settings.value("abinit/Tolmxf",
                                              "5.e-5").toString());
    setTolmxf();
    ui.dilatmxLineEdit->setText(settings.value("abinit/Dilatmx",
                                               "1.05").toString());
    setDilatmx();
    ui.ecutsmLineEdit->setText(settings.value("abinit/Ecutsm",
                                              "0.5").toString());
    setEcutsm();
    ...
    ui.shiftk1LineEdit->setText(settings.value("abinit/Shiftk1",
                                               "0.0").toString());
    setShiftk1();
    ui.shiftk2LineEdit->setText(settings.value("abinit/Shiftk2",
                                               "0.0").toString());
    setShiftk2();
    ui.shiftk3LineEdit->setText(settings.value("abinit/Shiftk3",
                                               "0.0").toString());
    setShiftk3();
    ui.toleranceTypeCombo->setCurrentIndex
      (settings.value("abinit/ToleranceType", 0).toInt());
    setToleranceType(settings.value("abinit/ToleranceType", 0).toInt());
    ui.tolXXXLineEdit->setText(settings.value("abinit/TolXXX",
                                              "1.e-10").toString());
    setTolXXX();
    // PAW Tab
    ui.pawecutdgLineEdit->setText(settings.value("abinit/Pawecutdg",
                                                 "20.0").toString());
    setPawecutdg();
  ...
  void AbinitInputDialog::updatePreviewText()
  {
    ...
    if (m_dirty && !m_warned) {
      ...
      msgBox.setText(tr("Would you like to update the preview text, "
                        "losing all changes made in the Abinit input "
                        "deck preview pane?"));
...
// Fragment copied from InputDialog
  ...
  void AbinitInputDialog::computeClicked()
  {
    if (m_process != 0) {
      QMessageBox::warning(this, tr("Abinit Running."),
                           tr("Abinit is already running. Please wait "
                              "until the previous calculation is finished."));
      return;
    }
    ...
    foreach (int zz, m_znucl) {
      ...
      pseudoFileName = QFileDialog::getOpenFileName(this, pseudoDialogTitle,
          defaultFileName, tr("Pseudos (*.hgh *.gth *.pspgth *.fhi *.pspnc "
                              "*.paw *.mt *.UPF *.upf *.psp)"));
      // this makes sure the next psp will be searched for in the same directory
      defaultFileName = pseudoFileName;
      filesFile = filesFile + pseudoFileName + "\n";
      iz++;
    }

    QString filesFileName = inputFile.canonicalPath() + '/'
      + inputFile.baseName() + ".files";
    ...
    if (!m_process->waitForStarted()) {
      QMessageBox::warning(this, tr("abinit failed to start."),
                           tr("abinit did not start. Perhaps it is "
    ...
    // print out log std output in a window... does not work for the
    // moment - do I need a window object to embed this in?
  ...
  void AbinitInputDialog::finished(int exitCode)
  {
    ...
    if (exitCode) {
      QMessageBox::warning(this, tr("Abinit Crashed."),
                           tr("Abinit did not run correctly. Perhaps "
    ...
    QString outputFile = inputFile.canonicalPath() + '/'
      + inputFile.baseName() + ".o";
    // for a relaxation run, need to add the _TIM10_ string too
    QString cmlFile = inputFile.canonicalPath() + '/'
      + inputFile.baseName() + "out.cml";
  ...
  {
    bool ok;
    double d = ui.ecutLineEdit->text().toDouble(&ok);
  ...
  {
    bool ok;
    double d = ui.tsmearLineEdit->text().toDouble(&ok);
    if (ok && d > 1.e-10)
      m_tsmear = d;
    updatePreviewText();
  }
  ...
  // TODO : check that nban 0 gives default behavior in abinit of
  // getting nelec/2 + 10% or so
  ...
  {
    bool ok;
    double d = ui.tolmxfLineEdit->text().toDouble(&ok);
    if (ok && d > 0.0)
      m_tolmxf = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.dilatmxLineEdit->text().toDouble(&ok);
    if (ok && d > 1.0)
      m_dilatmx = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.ecutsmLineEdit->text().toDouble(&ok);
    if (ok && d > 0.0)
      m_ecutsm = d;
    updatePreviewText();
  }
  ...
  // TODO : this should influence the number of lines of shiftk we
  // accept. Do not know how to do this in Qt
  ...
  {
    bool ok;
    double d = ui.shiftk1LineEdit->text().toDouble(&ok);
    if (ok && abs(d) <= 1.0)
      m_shiftk1 = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.shiftk2LineEdit->text().toDouble(&ok);
    if (ok && abs(d) <= 1.0)
      m_shiftk2 = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.shiftk3LineEdit->text().toDouble(&ok);
    if (ok && abs(d) <= 1.0)
      m_shiftk3 = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.tolXXXLineEdit->text().toDouble(&ok);
    if (ok && d > 0.0)
      m_tolXXX = d;
    updatePreviewText();
  }
  ...
  {
    bool ok;
    double d = ui.pawecutdgLineEdit->text().toDouble(&ok);
    if (ok && d > 0.0)
      m_pawecutdg = d;
    updatePreviewText();
  }
  ...
  QString AbinitInputDialog::generateInputDeck()
  {
    ...
    //  for information: this should be used when calling abinit with
    //  mpirun or something
