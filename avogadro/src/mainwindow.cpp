...
// Omit "common image formats" on Mac
    ...
    d->glWidget->raise();
    d->glWidget->repaint();
    exportImage = d->glWidget->grabFramebuffer();
  ...
  bool MainWindow::pasteMimeData(const QMimeData *mimeData, bool *specificError)
  {
    ...
    foreach (const ChemDrawCandidate &candidate, chemDrawCandidates) {
      ...
      const bool parseSucceeded = tryReadClipboardPayloadAsFormat(
        newMol, candidate.payload, candidate.formatId, &readerAvailable);
    ...
    } else if (!pasteFormatId.isEmpty()) {
      bool readerAvailable = false;
      validMol = tryReadClipboardPayloadAsFormat(
        newMol, text, pasteFormatId, &readerAvailable);
      if (!readerAvailable) {
        if (specificError)
          *specificError = true;
                                              ...
                                              const char* format)
  {
    ...
    if (!format) {
      ...
      d->glWidget->raise();
      d->glWidget->repaint();
      clipboardImage = d->glWidget->grabFramebuffer();
    }
